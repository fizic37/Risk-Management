...
#'
#' @noRd 
mod_read_excel_server <- function(input, output, session,excel_reactive, green="#20c997", red="#c92052"){
  # gets a reactiveValues() list with compononent file_input
  ns <- session$ns
  library(magrittr)
  observeEvent(excel_reactive$file_input,{
    ...
    if (length(sheets_read())>1) {
      ...
      shinyWidgets::inputSweetAlert(session = session,inputId = session$ns("ok_sheet"), input = "select",
              inputOptions = sheets_read(),type = "warning", btn_colors = green,btn_labels = "OK",
