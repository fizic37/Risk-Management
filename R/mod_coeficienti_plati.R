...
mod_coeficienti_plati_ui <- function(id){
  ns <- NS(id)
  tagList(
    bs4Dash::box(title = "Data raport plati",width = 12,collapsible = FALSE,status = "danger",
    ...
    bs4Dash::box(title = "Grad anual de recuperare creante nete",status = "info",width = 12,collapsible = T,collapsed = T,
...
mod_coeficienti_plati_server <- function(input, output, session, plati_reactive){
  ...
  updateSelectInput(session = session, inputId = "data_raport_plati",
                    choices = plati_reactive$view_sumar_plati$data_raport)
  ...
  output$sumar_creante <- DT::renderDataTable({
      DT::datatable(data = output_recuperari_plati(), caption = caption_baza_plati_nete(), extensions = "Buttons",rownames = FALSE,
                options = list(dom = "Bt", buttons = c("excel", "csv"))) %>% DT::formatRound(2:3,digits = 0) %>% 
  ...
  output$sumar_recuperari_ctg <- DT::renderDataTable({
    DT::datatable(data = sumar_recuperari_ctg(), caption = caption_recuperari_frc(), extensions = "Buttons",rownames = FALSE,
                  options = list(dom = "Bt", buttons = c("excel", "csv"))) %>% DT::formatRound(2,digits = 0) %>% 
      DT::formatPercentage(3:4,digits = 1)
